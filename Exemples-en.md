...
function foo() {
    ...
    //plays the 'background_music.mp3' file from the audio folder
    h.playAudio('background_music.mp3');

    //example if you have a folder with several audios and you want to play one randomly
    /*
     var player = h.getPlayer();
     player.setShuffle(true);
     h.playAudio('background_music'); //folder name
    */
...
function foo() {
    ...
    for (var k in arr) {
        ...
        if (json.data.length == 0) {
            ...
            //if it finds more than one result, it will display a window for the user to choose an item
            //creating the 'label' property as it will be displayed in the list as the item name
...
# OBS - List and change scene ws v5
Example of how to make a direct request to obs to obtain the list of available scenes, display the list for the user to choose a scene and change to the selected scene
